...
def HeuristicEvaluations(board, player, computer):
    score = 0
    centre_col = 3
    ...
    for condition in win_conditions:
        ...
        if player_count > 0 and computer_count == 0:
            score -= 10 ** player_count  # Penalty for computer, encourages blocking winning moves, exponents used to HIGHLY Penalise or Reward
        elif computer_count > 0 and player_count == 0:
            score += 10 ** computer_count  # Reward for computer, encourages winning moves
    return score

def MinimaxAlphaBeta(board, depth, alpha, beta, MaximisingPlayer, player, computer, win_conditions):
    if depth == 0 or check_win(board, win_conditions, computer) or check_win(board, win_conditions, player) or all(board[col][0] != ' ' for col in range(7)):
            ...
            if board[col][0] == ' ':
                for row in reversed(range(6)):
                    if board[col][row] == ' ': # Finds empty cell
                        ...
                        eval = MinimaxAlphaBeta(board, depth-1, alpha, beta, False, player, computer) # Recursion
                        '''
        ...
        if board[col][0] == ' ':
            for row in reversed(range(6)):
                if board[col][row] == ' ':
                    ...
                    score = MinimaxAlphaBeta(board, MAX_DEPTH, float('-inf'), float('inf'), False, player)
                    board[col][row] = ' '
